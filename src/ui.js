...
export const ui = {
    ...
    async init( fs )
    {
        ...
        this.currentView = null; // 'home' | 'explore' | 'view' | 'create' | 'user' | 'liked'
        ...
        if ( !mobile )
        {
            menubarOptions.push(
                {
                    name: 'Create',
                    callback: ( k, entry, event ) => this._openShader( 'new', event )
                },
                {
                    name: 'Explore',
                    callback: ( k, entry, event ) => this._openPage( `${k.toLowerCase()}/`, event )
                },
                {
                    name: 'Docs',
                    callback: ( k, entry, event ) => this._openPage( `${k.toLowerCase()}/`, event )
        ...
        // Do it always and use it for small screens
        {
            const sidebarOptions = {
                ...
                headerImage: this.dbUser?.avatar ?? `${ShaderHub.imagesRootPath}/favicon.png`,
                skipFooter: true,
                collapsed: false,
                collapsable: false,
        ...
        LX.addSignal( '@on_new_color_scheme', ( el, value ) => {
            if ( !mobile )
            {
                menubar.setButtonImage( 'ShaderHub', `${ShaderHub.imagesRootPath}/icon_${value}.png`, null, { float: 'left' } );
            }
        } );
    ...
    async router()
    {
        ...
        const isGraphicsPath = ( path ) => path.startsWith( '/view' ) || path.startsWith( '/create' );
    ...
    _makeFooter( area )
    {
        LX.makeContainer( [ `auto`, 'auto' ], 'text-foreground text-sm flex flex-row gap-2 self-center items-center text-center place-content-center', `
            ${
            LX.makeIcon( 'Github@solid', { svgClass: 'lg' } ).innerHTML
        }<a class="text-primary decoration-none hover:underline underline-offset-4" href="https://github.com/upf-gti/ShaderHub">Code on Github</a>`, area );
    },
    ...
    _generateShaderItemSkeleton( n, homeView )
    {
        ...
        for ( let i = 0; i < n; ++i )
        {
            const shaderItem = LX.makeElement( 'li',
                `shader-item ${homeView && i === 0 ? 'featured' : ''} lexskeletonpart relative hub-background-blur hover:bg-accent/50 overflow-hidden flex flex-col h-auto` );
    ...
    async renderHomePage()
    {
        ...
        let leftSide = LX.makeContainer( [ 'auto', '100%' ], 'bg-none flex flex-col p-6 gap-2 overflow-scroll items-center', '', topArea );
        leftSide.style.minWidth = '50%';
        let rightSide = LX.makeContainer( [ '100%', '100%' ], 'bg-none flex flex-col p-6 md:p-12 place-content-center items-center', '', topArea );

        // Create title/login area
        {
            const container = LX.makeContainer( [ '100%', '100%' ], 'hub-background-blur flex flex-col gap-4 rounded-xl py-12 box-shadow box-border justify-evenly overflow-scroll items-center', '',
                rightSide );
            ...
            if ( this.fs.user )
            {
                ...
                const welcomeMessage = LX.makeContainer( [ '100%', '15%' ], 'px-8 text-center text-3xl text-card-foreground content-center', `Welcome ${dbUser.display_name ?? dbUser.user_name}!`,
                    container );
            ...
            const header = LX.makeContainer( [ null, 'auto' ], 'flex flex-col px-8 md:px-12 gap-12 text-center items-center place-content-center', `
                ...
                <span class="flex flex-auto-fill">Shader Collections, Snippet Library, and New Code editor features</span>${
                LX.makeIcon( 'ArrowRight', { svgClass: 'flex flex-auto-keep sm' } ).innerHTML
            }</a>
            `, container );
            ...
            if ( !mobile )
            {
                ...
                const getStartedButton = new LX.Button( null, 'Get started now', () => this._openShader( 'new' ), { icon: 'ChevronRight', iconPosition: 'end', buttonClass: 'lg primary' } );
        ...
        LX.makeContainer( [ '100%', 'auto' ], 'font-medium text-card-foreground mb-2 text-3xl', `Featured Shaders`, leftSide );

        const numFeatured = 5;
        const skeletonHtml = this._generateShaderItemSkeleton( numFeatured, true );
        const skeleton = new LX.Skeleton( skeletonHtml );
        LX.addClass( skeleton.root, 'grid shader-list-initial gap-8 justify-center max-w-[740px]' );
                    ...
                    !shader.anonAuthor ? `<a onclick='ui._openUserProfile("${shader.authorId}")' class='hub-link font-medium'>` : ''
                        ...
                        <div class="flex flex-row gap-2 items-center ml-auto flex-auto-keep">
                            <div class="flex flex-row gap-1 items-center">
                                ${
                    LX.makeIcon( 'Heart', {
                        svgClass: `shader-like-button ${
                            shader.liked ? 'text-orange-600 shadow-primary' : 'text-muted-foreground'
                        } fill-current sm hover:text-orange-600 hover:shadow-primary cursor-pointer`
                    } ).innerHTML
                }
    ...
    async renderUserView( userID )
    {
        ...
        const tabs = topArea.addTabs( { parentClass: 'bg-transparent flex justify-center', sizes: [ 'auto', 'auto' ], contentClass: 'p-2 my-2 bg-transparent rounded-xl border-color h-auto!' } );
        ...
        // Shader list
        {
            ...
            // Profile Shader Order
            {
                ...
                {
                    const fLower = f.toLowerCase();
                    filtersPanel.addButton( null, f, ( v ) => this._exploreOrderBy( v.toLowerCase(), 'user' ), {
                        buttonClass: `xs ${currentOrderFilter === fLower ? 'primary' : 'outline bg-card!'}`
                    } );
                }
            ...
            this._refreshOwnShaders = async ( filterSearch, filterPage ) => {
                ...
                if ( dbShaders.length === 0 )
                {
                    ...
                    const getStartedButton = new LX.Button( null, 'Create a Shader', () => this._openShader( 'new' ), { className: 'mt-2 place-self-center w-fit!', icon: 'ChevronRight',
                        iconPosition: 'end', buttonClass: 'lg primary' } );
                    headerButtons.appendChild( getStartedButton.root );
                    return;
                }
                ...
                LX.doAsync( async () => {
                    ...
                    for ( let i = 0; i < dbShaders.length; ++i )
                    {
                        ...
                        shaderPreview.onload = () => shaderPreview.classList.remove( 'opacity-0' );
                        shaderItem.querySelector( 'div' ).remove();
                        let visibilityChip = null;
                        if ( !shaderInfo.public )
                        {
                            visibilityChip = LX.makeElement( 'span', 'absolute flex flex-row gap-1 m-4 bg-background-blur border-color text-xs px-2 py-1 rounded-xl select-none pointer-events-none',
                                `${LX.makeIcon( 'Lock', { svgClass: 'sm' } ).innerHTML} Private`, shaderItem );
                        }
                        const shaderDesc = LX.makeContainer( [ '100%', 'auto' ], `flex ${ownProfile ? 'flex-col' : 'flex-row'} rounded-b-lg gap-1 px-4 py-3 items-center select-none`, `
                            ...
                            <div class="flex flex-row gap-1 flex-auto-keep items-center shader-prof-opt">
                                <div class="flex flex-row gap-2 items-center">
                                    ...
                                    <div class="flex flex-row gap-1 items-center">
                                        ${LX.makeIcon( 'Eye', { svgClass: `text-muted-foreground fill-current sm` } ).innerHTML}
                                        <span class="text-xs text-muted-foreground">${shaderInfo.viewCount ?? 0}</span>
                                    </div>
                                </div>
                        ...
                        if ( ownProfile && shaderOptionsCont )
                        {
                            ...
                            }, { icon: shaderInfo.public ? 'Globe' : 'Lock', swap: shaderInfo.public ? 'EyeOff' : 'Eye', title: 'Toggle Public/Private', tooltip: true, className: 'p-0',
                                buttonClass: 'px-1! outline sm' } );
                            ...
                            }, { icon: 'Layers2', title: 'Collections', tooltip: true, className: 'p-0', buttonClass: 'px-1! outline sm' } );
        ...
        // Collections
        {
            ...
            tabs.add( 'Collections', collectionsContainer, { xselected: true, onSelect: async ( event, name ) => {
                ...
                collections.documents.forEach( ( c ) => {
                    const collectionContainer = LX.makeContainer( [ '100%', 'auto' ], 'hub-background-blur flex flex-col gap-1 text-2xl border-color rounded-xl p-6',
                        `<span class="mb-2">${c.name}</span>`, content );
        ...
        // Preferences
        {
            ...
            // profile
            {
                ...
                let userAvatar = user.avatar ?? '', userDescription = user.description ?? '', userDisplayName = user.display_name ?? '', userAppMode = user.app_mode ?? 'Auto',
    ...
    async renderLikedView()
    {
        ...
        document.title = `Shader likes - ShaderHub`;
        ...
        if ( !this.fs.user || !this.dbUser )
        {
            LX.makeContainer( [ '100%', 'auto' ], 'mt-8 text-2xl font-medium justify-center text-center', 'Sign in to see your liked shaders.', topArea );
            return;
        }

        LX.makeContainer( [ '100%', 'auto' ], 'font-medium text-card-foreground text-4xl text-center mb-2', `Liked`, topArea );

        const PAGE_LIMIT = 8;
        const usersDocuments = await this.fs.listDocuments( FS.USERS_COLLECTION_ID, [ Query.limit( 1e3 ) ] );
        const user = this.dbUser;
        const likesContainer = LX.makeContainer( [ null, 'auto' ], 'flex flex-col relative p-1 pt-0 rounded-lg overflow-hidden', '', topArea );
        ...
        const paginator = new LX.Pagination( {
            ...
            xallowChangeItemsPerPage: true,
            onChange: ( page ) => {
                this._refreshLikedShaders( page.toString() );
            }
        } );
        listHeader.appendChild( paginator.root );
        ...
        const exploreMoreBtn = new LX.Button( null, 'Explore more shaders', () => this._openPage( 'explore/' ), { className: 'mt-2 place-self-center w-fit!', icon: 'ArrowRight', iconPosition: 'end',
            buttonClass: 'lg link' } );
        topArea.attach( exploreMoreBtn );

        this._refreshLikedShaders = async ( filterPage ) => {
            listContent.innerHTML = '';
            ...
            let dbShaders = result.documents;
            if ( dbShaders.length === 0 )
            {
                        ...
                        header: {
                            ...
                            action: { name: snippet.options ? 'Open' : 'Insert', callback: () => {
                                ...
                                else
                                {
                                    ...
                                    const code = ShaderHub.getSnippetCode( snippet, usingWebGPU );
                                    if ( editor ) editor.appendText( `${code}\n` );
                                }
                            } }
                        },
                        className: 'h-full'
        ...
        this._createShaderDataView = async () => {
            ...
            if ( this.fs.user )
            {
                ...
                if ( mobile )
                {
                    if ( result )
                    {
                        const shareShaderButton = new LX.Button( null, 'ShareShaderButton', async () => this.openShareiFrameDialog( result ), { icon: 'Share2', buttonClass: 'primary' } );
                ...
                else
                {
                    ...
                    {
                        const saveShaderButton = new LX.Button( null, 'SaveShaderButton', async () => ShaderHub.saveShader( result ), { icon: 'Save', buttonClass: 'primary',
                            title: result ? 'Update Shader' : 'Save Shader', tooltip: true } );
                    ...
                    if ( !isNewShader && result )
                    {
                        const shaderOptionsButton = new LX.Button( null, 'ShaderOptions', async () => {
                            ...
                            if ( ownProfile )
                            {
                                dmOptions.push( null, { name: 'Delete Shader', icon: 'Trash2', className: 'destructive', callback: () =>
                                    ShaderHub.deleteShader( undefined, () => {
                ...
                // Comments
                {
                    ...
                    const refreshComments = async () => {
                        ...
                        if ( canComment )
                        {
                            ...
                            const commentInput = new LX.TextArea( null, '', null, { placeholder: 'Add a comment...', className: 'flex w-full flex-auto-fill', inputClass: 'bg-background/50!',
                                fitHeight: true } );
                        ...
                        if ( shaderComments.total > 0 )
                        {
                            ...
                            for ( const comment of shaderComments.documents )
                            {
                                ...
                                const commentDate = ( new Date( comment['$createdAt'] ) ).toLocaleDateString( undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric',
                                    minute: 'numeric' } );
                                        ...
                                        const options = [
                                            {
                                                ...
                                                callback: () => {
                                                    ...
                                                    const newReplyItem = LX.makeContainer( [ `100%`, 'auto' ],
                                                        'shader-comment-reply-item flex flex-row mt-2 p-2 gap-2 bg-muted/75 rounded-lg items-start content-center', ``, commentItemContainer );

                                                    const replyInput = new LX.TextArea( null, '', ( v ) => {
                                                        if ( v.length === 0 ) newReplyItem.remove();
                                                    }, { placeholder: `Replying to ${commentAuthorName}...`, className: 'flex w-full flex-auto-fill', inputClass: 'bg-background/50!',
                                                        fitHeight: true } );
                                ...
                                for ( const reply of revReplies )
                                {
                                    ...
                                    const replyDate = ( new Date( reply['$createdAt'] ) ).toLocaleDateString( undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric',
                                        minute: 'numeric' } );
                    ...
                    LX.addDropdownMenu( button, [
                        {
                            ...
                            submenu: [ 'gif', 'png', 'webm' ].map( ( o ) => {
                                ...
                                return { name: `${checked ? LX.makeIcon( 'Circle', { svgClass: '2xs fill-current inline-flex! mr-2' } ).innerHTML : ''}${o}`, callback: ( v ) =>
                                    iUpdateExportOptions( 'format', v ) };
                            } )
                        },
                        {
                            name: 'Frames',
                        ...
                        },
                        {
                            ...
                            submenu: [ 10, 15, 30, 60 ].map( ( o ) => {
                                ...
                                return { name: `${checked ? LX.makeIcon( 'Circle', { svgClass: '2xs fill-current inline-flex! mr-2' } ).innerHTML : ''}${o}`, callback: ( v ) =>
                                    iUpdateExportOptions( 'framerate', v ) };
                            } )
                        }
    ...
    async makeStatusBarButtons( p, editor )
    {
        ...
        const openQuickHelp = () => {
            const dialog = new LX.Dialog( 'Quick Help', ( p ) => {
                ...
                makeParagraph(
                    `ShaderHub is a playground for both WebGPU render and compute shaders. Everything here is written in WGSL, which is WebGPU's native shader language. For up-to-date information on WGSL, please see the <a href='https://www.w3.org/TR/WGSL/'>WGSL draft specification</a>. You can also take a <a href='https://google.github.io/tour-of-wgsl/'>tour of WGSL</a>.`,
                    p
                );
...
<span class="text-foreground">iRefreshRate: i32</span> <span class="text-muted-foreground">// FPS</span><br>
...
<span class="text-foreground">textureSample(iChannel0...3, <span class="text-destructive font-semibold">sampler</span>, uv)</span>`, p );
    ...
    <li><span class="text-destructive font-semibold">SCREEN_WIDTH</span> and <span class="text-destructive font-semibold">SCREEN_HEIGHT</span> are predefined variables for accessing canvas dimensions</li>
        ...
        <li><span class="text-destructive font-semibold">#storage NAME TYPE</span> for declaring a storage buffer</li>
                ...
                p.addTitle( 'Examples' );
                makeParagraph(
                    `<a href="/?shader=68b8931090e336e8a1ad" class="text-foreground decoration-none font-semibold hover:text-card-foreground underline-offset-4 hover:underline cursor-pointer">Custom Uniforms</a>`,
                    p
                );
                makeParagraph(
                    `<a href="/?shader=68c449875d3a5535bba0" class="text-foreground decoration-none font-semibold hover:text-card-foreground underline-offset-4 hover:underline cursor-pointer">Texture Buffer pass</a>`,
                    p
                );
                ...
                {
                    makeParagraph(
                        `<a href="/?shader=68dac43a64cff0f5fa9d" class="text-foreground decoration-none font-semibold hover:text-card-foreground underline-offset-4 hover:underline cursor-pointer">Simple Compute Pass</a>`,
                        p
                    );
                    makeParagraph(
                        `<a href="/?shader=68da7c00ef76c57eb056" class="text-foreground decoration-none font-semibold hover:text-card-foreground underline-offset-4 hover:underline cursor-pointer">Compute Storage usage</a>`,
                        p
                    );
                }
    ...
    pushCompilationLog( passName, msgType, msgText, lineNumber, colNumber )
    {
        ...
        const entry = LX.makeContainer( [ '100%', 'auto' ],
            `flex flex-row items-center gap-2 px-3 py-2 rounded-md cursor-pointer text-sm hover:bg-accent transition ${isError ? 'text-destructive' : 'text-warning'}`, `
    ...
    async openCurrentShaderLikesDialog( shader )
    {
        ...
        if ( shaderLikes.total === 0 )
        {
            const dialog = new LX.Dialog( null, ( p ) => {
                ...
                p.addTextArea( null, `Share this shader so other people can check it and leave a like!`, null, { disabled: true, fitHeight: true,
    ...
    openShareiFrameDialog( r )
    {
        ...
        const dialog = new LX.Dialog( 'Share this Shader', ( p ) => {
            ...
            // iframe code
            {
                ...
                const iframeUrl = `<iframe src="${window.location.origin}${window.location.pathname}embed/?shader=${r['$id']}${
                    showUI ? '' : '&ui=false'
                }" frameborder="0" width="640" height="405" class="rounded-lg" allowfullscreen></iframe>`;
        ...
        const _createChannelItems = async ( category, container ) => {
            ...
            for ( const document of assets )
            {
                ...
                else
                {
                    ...
                    const url = localUrl ?? ( preview ? await this.fs.getFileUrl( preview ) : ( fileId ? await this.fs.getFileUrl( fileId ) : ShaderHub.shaderPreviewPath ) );
                ...
                const shaderDesc = LX.makeContainer( [ '100%', 'auto' ],
                    'absolute text-xs top-0 p-2 w-full rounded-t-lg bg-background-blur backdrop-blur-xs items-center select-none font-semibold keep-all', `
                    ${document.name} (uint8)
                `, channelItem );
    ...
    async updateShaderChannelsView( pass, channel )
    {
        ...
        const iUpdateChannel = async ( channelIndex ) => {
            ...
            const channelContainer = LX.makeContainer( [ '100%', '100%' ],
                'relative text-center content-center box-shadow box-border rounded-lg bg-card hover:bg-accent cursor-pointer overflow-hidden', '' );
            ...
            {
                // Channel Options
                const channelOptions = LX.makeContainer( [ '100%', 'auto' ],
                    'flex flex-row absolute bg-background-blur backdrop-blur-xs text-xs text-center content-center justify-end bottom-0 rounded-b-lg', '', channelContainer );
